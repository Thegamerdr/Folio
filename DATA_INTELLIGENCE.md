...
- 07-06: PHASE 4 SHIPPED + OTA'd (historic backfill, 1201 tests). Discovery caught + fixed pre-ship: BST month-shift TZ bug (verified on-box), reconstructed months polluting headline averages, partial first month, unbounded cycles. Phase 5 launched.
- 07-06: PHASE 5 SHIPPED (caught-bills + weekly-cadence unlock, 1231 tests). (A) `caughtSubs.ts` SHEET_CADENCE widened to weekly/fortnightly/monthly (quarterly/yearly stay out); candidates now carry `cadence`; SubCaughtSheet copy + "Seen N <unit> in a row" meta line made cadence-aware via `copy.subs.caught.body(cadence)`, confirm mechanics untouched. (B) New `caughtBills.ts` mirrors caughtSubs.ts over the same `detectRecurring` engine. REAL-ENTITY DECISION (recorded in the module header): no separate bill entity exists in the live spine — `AddEntryScreen`'s own "Add a bill" flow already writes `setSubs`, so a caught bill also confirms into `subs[]`, the same catalog a caught sub does; `RECURRING_BILLS`/`CalendarEvent` were investigated and rejected (demo-hardcoded / no cadence field respectively). Double-propose guard: excludes only the ONE merchant findCaughtSubs currently offers (`caught[0]`), not its full output — `detectRecurring` has no sub/bill distinction, so excluding the full set would leave findCaughtBills permanently empty. New `dismissedBillSignals` store slice + `dismissBillSignal` action mirror the income pattern exactly. (C) New `BillCaughtSheet.tsx` mirrors IncomeCaughtSheet/SubCaughtSheet (`bill-caught` SheetId registered in types.ts + FolioShell.tsx); wired at the same two call sites as income-caught (VisualizerScreen.commit, ReviewScreen.onAdd) — income proposal takes precedence when both fire on one landing (one sheet per landing); a qualifying bill simply re-evaluates fresh on the next landing rather than being lost. Discovery: the "subs take precedence" fixture trap — a test ledger with only ONE qualifying merchant makes it findCaughtSubs's sole candidate by construction, so bill-positive test fixtures need a second "sink" merchant landed in the right batch order to avoid tripping the guard on themselves (documented in both new test files). Phase 6 (history-fed forecasts) next.
- 07-06: PHASE 5 SHIPPED + OTA'd (caught bills + weekly unlock + renewal math, 1238 tests). Discovery: Shield-escape renewal bug fixed pre-ship; annual-bill radar + council-tax holiday folded into phase 6; transfer-rent + amount-band notes to backlog. Phase 6 launched.
- 07-06: PHASE 6 SHIPPED + OTA'd (1309 tests). Discovery hardening pre-ship: drift cooldown, annual-subs guard, quiet-moment gate. Perf note: 4 detector passes per landing at 2000 rows = acceptable ms-scale, memoization deferred. Phase 7 launched.
- 07-06: PHASE 7 SHIPPED + OTA'd (chunked reading, 1344 tests). Live-proven on the owner's 133-page export. Discovery hardening: queue spillover, preview id keys, scope line. Same-day-identical-txn dedupe tradeoff documented -> backlog. Phase 8 launched.
- 07-06: PHASE 8 SHIPPED (consolidation safe pass, 1335 tests). No OTA (no user-facing change). CONSOLIDATION.md carries the (c) items: kit.tsx wrinkle extraction, 11k-line dead tree, live-duplicate formula migrations.
- 07-06: PHASE 9 SHIPPED + OTA'd (1387 tests). Sweep CRITICAL (chat frozen numbers) fixed pre-ship. Remaining documented: CONSOLIDATION.md (c) items, RNTL next-steps, dead-tree decision.
