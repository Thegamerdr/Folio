...
| Clean reset and seed repeatability       | P0       | repo fix landed + tested, device proof pending     | Owner must be able to reset and repeat scenarios without stale data.        | Dogfood reset screenshot, empty-state proof after reset                  | `apps/mobile/src/folio/store.ts` (`resetToEmpty`), More/Privacy/Account screens (`dogfoodMode.ts` is held-back, NOT shipped) | yes      | helpful         | no           | no              | Verify the shipped reset (More/Privacy "start fresh"/clean-slate paths → `resetToEmpty`) on the phone: after reset the app shows a genuinely empty state, no demo reseed. Repo evidence: `store.test.ts` resetToEmpty suite + `PrivacyScreen.cleanSlate.test.ts` + marker-based seed containment (`isRealUser`/`purgeSeedIfReal`). |
...
| Local storage and key behavior on device | P0       | repo hardened + partial device proof (2026-07-11)  | Owner data should not silently fall back or vanish unexpectedly.            | restart persistence, clear-data proof, key-loss behaviour                | `apps/mobile/src/folio/lib/persist.ts` + `vaultKey.ts` (expo-secure-store) — the shipped stack; SQLCipher rows belong to the held-back layer | partly   | yes             | no           | no              | Landed: atomic+serialized blob writes, proven-good backup, parked unreadable blob, degraded-load→backup recovery (plans 101/102, 10-scenario `persistRecovery.test.ts`). Device proof so far: in-place upgrade + relaunch showed the owner's real data intact. Still owed: clear-data/reinstall behaviour + key-loss (secure-store wipe) drill. |
