...
export function normaliseBusinessOperationsState(
  ...
  return {
    ...
    saPotMinor: safeMinor(value.saPotMinor),
...
export type BusinessInsightsPeriod = Readonly<{
  start: string;
  end: string;
  label: string;
}>;

export type BusinessPeriodPnl = Readonly<{
  period: BusinessInsightsPeriod;
  revenueMinor: number;
...
function obligationMonthlyMinor(obligation: BusinessObligation): number {
  if (obligation.cadence === 'monthly') return obligation.amountMinor;
  if (obligation.cadence === 'quarterly') return obligation.amountMinor / 3;
  if (obligation.cadence === 'annual') return obligation.amountMinor / 12;
  return (obligation.amountMinor * 52) / 12;
}

export function businessPeriodPnl(
  state: BusinessOperationsState,
  period: BusinessInsightsPeriod,
): BusinessPeriodPnl {
  const start = dateMs(period.start);
  const end = dateMs(period.end);
  const paid = state.invoices.filter((invoice) => {
    if (!invoice.paidOn) return false;
    const paidAt = dateMs(invoice.paidOn);
    return paidAt >= start && paidAt <= end;
  });
  const revenueMinor = paid.reduce(
    (sum, invoice) => sum + Math.max(0, safeMinor(invoice.paidMinor)),
    0,
  );
  const windowDays = Math.max(1, Math.round((end - start) / DAY_MS));
  const expensesMinor = state.obligations.reduce(
    (sum, obligation) => sum + Math.round(obligationMonthlyMinor(obligation) * (windowDays / 30)),
    0,
  );
  return {
    period,
    revenueMinor,
    expensesMinor,
    netMinor: revenueMinor - expensesMinor,
    paidInvoiceCount: paid.length,
    averageInvoiceMinor: paid.length > 0 ? Math.round(revenueMinor / paid.length) : 0,
  };
}

export type BusinessClientInsight = Readonly<{
  client: string;
  paidMinor: number;
...
export function businessTopClients(
  state: BusinessOperationsState,
  limit = 5,
): readonly BusinessClientInsight[] {
  const rows = new Map<string, BusinessClientInsight>();
  for (const invoice of state.invoices) {
    const client = invoice.clientName.trim() || 'Unnamed';
    const current = rows.get(client) ?? {
      client,
      paidMinor: 0,
      outstandingMinor: 0,
      invoiceCount: 0,
    ...
    rows.set(client, {
      client,
      paidMinor: current.paidMinor + Math.max(0, safeMinor(invoice.paidMinor)),
      outstandingMinor: current.outstandingMinor + outstandingInvoiceMinor(invoice),
  ...
  return [...rows.values()]
    .sort(
      (left, right) =>
        right.paidMinor + right.outstandingMinor - (left.paidMinor + left.outstandingMinor),
    )
    .slice(0, limit);
}

export type BusinessCostInsight = Readonly<{
  category: string;
  monthlyMinor: number;
  annualMinor: number;
  count: number;
...
const BUSINESS_COST_LABEL: Readonly<Record<BusinessObligation['category'], string>> = {
  rent: 'Rent & premises',
  payroll: 'Payroll',
  software: 'Software & tools',
  tax: 'Tax standing orders',
  loan: 'Loan repayments',
  other: 'Other',
};

export function businessTopCosts(
  state: BusinessOperationsState,
  limit = 5,
): readonly BusinessCostInsight[] {
  const rows = new Map<BusinessObligation['category'], BusinessCostInsight>();
  for (const obligation of state.obligations) {
    const monthlyMinor = obligationMonthlyMinor(obligation);
    const current = rows.get(obligation.category) ?? {
      category: BUSINESS_COST_LABEL[obligation.category],
      monthlyMinor: 0,
      annualMinor: 0,
      count: 0,
    };
    rows.set(obligation.category, {
      ...current,
      monthlyMinor: current.monthlyMinor + monthlyMinor,
      annualMinor: current.annualMinor + monthlyMinor * 12,
  ...
  return [...rows.values()]
    .map((row) => ({
      ...row,
      monthlyMinor: Math.round(row.monthlyMinor),
      annualMinor: Math.round(row.annualMinor),
    }))
    .sort((left, right) => right.monthlyMinor - left.monthlyMinor)
    .slice(0, limit);
}

export type BusinessMonthPoint = Readonly<{
  key: string;
  label: string;
  revenueMinor: number;
}>;

export function businessMonthlyRevenue(
  state: BusinessOperationsState,
  months = 12,
  now = new Date(),
): readonly BusinessMonthPoint[] {
  const points: Array<{ key: string; label: string; revenueMinor: number }> = [];
  ...
  const indexByKey = new Map(points.map((point, index) => [point.key, index] as const));
  for (const invoice of state.invoices) {
    if (!invoice.paidOn) continue;
    const paidAt = new Date(`${invoice.paidOn}T00:00:00Z`);
    const key = `${paidAt.getUTCFullYear()}-${String(paidAt.getUTCMonth() + 1).padStart(2, '0')}`;
    const index = indexByKey.get(key);
    if (index !== undefined) {
      points[index]!.revenueMinor += Math.max(0, safeMinor(invoice.paidMinor));
    }
  }
  return points;
}

export type BusinessSeasonality = Readonly<{
  bestMonth: BusinessMonthPoint | null;
  quietMonth: BusinessMonthPoint | null;
  trend: 'up' | 'down' | 'flat';
}>;

export function businessSeasonality(points: readonly BusinessMonthPoint[]): BusinessSeasonality {
  if (points.length === 0) {
    return { bestMonth: null, quietMonth: null, trend: 'flat' };
  }
  const withRevenue = points.filter((point) => point.revenueMinor > 0);
  const bestMonth = withRevenue.reduce<BusinessMonthPoint | null>(
    (best, point) => (!best || point.revenueMinor > best.revenueMinor ? point : best),
    null,
  );
  const quietMonth = withRevenue.reduce<BusinessMonthPoint | null>(
    (quiet, point) => (!quiet || point.revenueMinor < quiet.revenueMinor ? point : quiet),
    null,
  );
  const half = Math.floor(points.length / 2);
  const average = (values: readonly number[]) =>
    values.length > 0 ? values.reduce((sum, value) => sum + value, 0) / values.length : 0;
  const firstAverage = average(points.slice(0, half).map((point) => point.revenueMinor));
  const lastAverage = average(points.slice(half).map((point) => point.revenueMinor));
  const delta = lastAverage - firstAverage;
  const trend = Math.abs(delta) < firstAverage * 0.1 ? 'flat' : delta > 0 ? 'up' : 'down';
  return { bestMonth, quietMonth, trend };
}

export function currentBusinessTaxYear(now = new Date()): BusinessInsightsPeriod {
  const year = now.getUTCFullYear();
  const cutover = Date.UTC(year, 3, 6);
  const startYear = now.getTime() >= cutover ? year : year - 1;
  return {
...
export type BusinessTaxYearStory = Readonly<{
  period: BusinessInsightsPeriod;
  pnl: BusinessPeriodPnl;
  daysElapsed: number;
  daysTotal: number;
  progressPct: number;
...
export function businessTaxYearStory(
  state: BusinessOperationsState,
  now = new Date(),
): BusinessTaxYearStory {
  const period = currentBusinessTaxYear(now);
  const today = utcDay(now).toISOString().slice(0, 10);
  const pnl = businessPeriodPnl(state, { ...period, end: today });
  const daysElapsed = Math.max(1, Math.round((dateMs(today) - dateMs(period.start)) / DAY_MS));
  const daysTotal = Math.round((dateMs(period.end) - dateMs(period.start)) / DAY_MS);
  ...
  return {
    ...
    daysTotal,
    progressPct,
    projectedRevenueMinor: Math.round((pnl.revenueMinor / daysElapsed) * daysTotal),
  };
}
