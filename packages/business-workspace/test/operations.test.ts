...
describe('2026/27 Business operations', () => {
  ...
  it('derives the live Business insights story from paid invoices and recurring costs', () => {
    const state = {
      ...emptyBusinessOperationsState(),
      invoices: [
        {
    ...
    expect(
      ...
      expensesMinor: 11_600,
      netMinor: 288_400,
      paidInvoiceCount: 1,
      averageInvoiceMinor: 300_000,
    });
    expect(businessTopClients(state)).toEqual([
      {
        client: 'Acme',
        paidMinor: 300_000,
    ...
    expect(businessTopCosts(state)[0]).toEqual({
      category: 'Software & tools',
      monthlyMinor: 12_000,
      annualMinor: 144_000,
      count: 1,
    });
    const months = businessMonthlyRevenue(state, 3, new Date('2026-07-19T12:00:00Z'));
    expect(months.map((point) => [point.key, point.revenueMinor])).toEqual([
      ['2026-05', 0],
      ['2026-06', 300_000],
      ['2026-07', 0],
    ]);
    expect(businessSeasonality(months)).toMatchObject({
      bestMonth: { key: '2026-06' },
      quietMonth: { key: '2026-06' },
      trend: 'up',
    });
    expect(businessTaxYearStory(state, new Date('2026-07-19T12:00:00Z'))).toMatchObject({
      period: { label: '2026/27' },
      pnl: { revenueMinor: 300_000 },
    });
  });
});
