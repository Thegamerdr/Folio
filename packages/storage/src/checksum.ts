...
export type Checksum = `sha256:${string}`;
...
export function createChecksum(input: string): Checksum {
  const digest = sha256Hex(normalizeLineEndings(input));
  return `sha256:${digest}`;
}

export function createJsonChecksum(input: JsonValue): Checksum {
  return createChecksum(stableStringify(input));
}

export function assertChecksum(input: string, expected: Checksum, label = 'checksum'): void {
  const actual = createChecksum(input);
  if (actual !== expected) {
...
function sha256Hex(input: string): string {
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 =
        (h + bigSigma1 + choose + (SHA256_ROUND_CONSTANTS[index] ?? 0) + (words[index] ?? 0)) >>> 0;
  ...
  for (let index = 0; index < input.length; index += 1) {
    ...
    if (codePoint >= 0xd800 && codePoint <= 0xdbff && index + 1 < input.length) {
      ...
      if (next >= 0xdc00 && next <= 0xdfff) {
        codePoint = 0x1_0000 + ((codePoint - 0xd800) << 10) + (next - 0xdc00);
        index += 1;
      }
    }
