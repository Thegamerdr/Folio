...
type SheetProps = {
  ...
  children: ReactNode;
  /**
   * Most sheets use the primitive's content scroller. Complex surfaces such as Melo chat already
   * own a fixed header, an independently scrolling transcript, and a keyboard-bound composer; an
   * additional outer ScrollView lets Android scroll the whole sheet to the focused input and can
   * push the header under the status bar.
   */
          ...
          >
            <Animated.View
              ...
              style={[
                ...
                {
                  maxHeight,
                  // Fixed-content sheets (Melo chat) fill the currently available avoider height,
                  // not the pre-keyboard window height. Android's `behavior="height"` shrinks the
                  // avoider when the IME opens; a numeric `maxHeight` here kept the panel taller
                  // than that shrunken space and pushed its header/transcript above the display.
            ...
            >
              ...
              {scrollable ? (
                ...
                <View style={[layout.scrollContent, layout.fixedContent]}>{children}</View>
              )}
            </Animated.View>
          </KeyboardAvoidingView>
        </View>
