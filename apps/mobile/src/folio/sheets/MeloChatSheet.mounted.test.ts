...
const props = (prefill = 'hello') => ({ visible:true, onClose:vi.fn(), nav:{go:vi.fn(),openSheet:vi.fn(),back:vi.fn()} as any, pressure:'steady' as any, intent:{prefill} as any });
const mountedTrees: renderer.ReactTestRenderer[] = [];
function renderChat(prefill = 'hello') { let tree!: renderer.ReactTestRenderer; const scrollNode = { scrollTo: vi.fn(), scrollToEnd: vi.fn() }; act(()=>{tree=renderer.create(React.createElement(MeloChatSheet,props(prefill)), { createNodeMock: (element: any) => element.type === 'ScrollView' ? scrollNode : null });}); mountedTrees.push(tree); return { tree, scrollNode }; }
