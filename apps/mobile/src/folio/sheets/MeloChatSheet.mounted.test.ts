...
describe('MF13 MeloChatSheet mounted interaction harness', () => {
  ...
  it('releases terminal alignment after send followed by reader scrollback',async()=>{ const {tree}=renderChat(); const input=tree.root.findByProps({accessibilityLabel:'Message Melo'}); const sheet=tree.root.findAll((node)=>(node.type as any)==='Sheet')[0]!; act(()=>input.props.onChangeText('a longer follow-up')); expect(sheet.props.terminalAlignmentEnabledRef.current).toBe(true); await act(async()=>{await labelled(tree,'Send message')[0]!.props.onPress();}); const scroll=tree.root.findAll((node)=>(node.type as any)==='ScrollView')[0]!; act(()=>scroll.props.onScroll({nativeEvent:{contentOffset:{y:120},contentSize:{height:1000},layoutMeasurement:{height:400}}})); expect(sheet.props.terminalAlignmentEnabledRef.current).toBe(false); act(()=>input.props.onFocus?.()); expect(sheet.props.terminalAlignmentEnabledRef.current).toBe(false); });
  it('does not force a typed draft back to the terminal after reader scrollback',()=>{ const {tree}=renderChat(); const input=tree.root.findByProps({accessibilityLabel:'Message Melo'}); const sheet=tree.root.findAll((node)=>(node.type as any)==='Sheet')[0]!; act(()=>input.props.onChangeText('draft kept while reading')); const scroll=tree.root.findAll((node)=>(node.type as any)==='ScrollView')[0]!; act(()=>scroll.props.onScroll({nativeEvent:{contentOffset:{y:96},contentSize:{height:960},layoutMeasurement:{height:400}}})); expect(sheet.props.terminalAlignmentEnabledRef.current).toBe(false); });
});
