...
function projectCompanionRuntimeState(
  ...
  repository.companionRuntimeStates.put({
    ...
    meloPrimerBeat: Math.max(0, Math.min(3, Math.floor(state.meloPrimerBeat ?? 0))),
    meloPrimerSeenAt:
      state.meloPrimerSeenAt === undefined || state.meloPrimerSeenAt === null
        ? null
        : canonicalInstant(state.meloPrimerSeenAt, 'Melo intro completion time'),
    lastOpenedAt:
    ...
    meloMoves: (state.meloMoves ?? []).map((move, index) => ({
      id: requireString(move.id, `Melo move ${index} ID`),
      createdAt: canonicalInstant(move.createdAt, `Melo move ${index} created time`),
      headline: requireString(move.headline, `Melo move ${index} headline`),
      kind: move.kind,
      status: move.status,
      ...(move.amount === undefined
        ? {}
        : { amount: requireFiniteNumber(move.amount, `Melo move ${index} amount`) }),
      ...(move.targetId === undefined
        ? {}
        : { targetId: requireString(move.targetId, `Melo move ${index} target ID`) }),
      ...(move.outcome === undefined
        ? {}
        : {
            outcome: {
              resolvedAt: canonicalInstant(
                move.outcome.resolvedAt,
                `Melo move ${index} outcome time`,
              ),
              pathDelta: requireFiniteNumber(
                move.outcome.pathDelta,
                `Melo move ${index} path delta`,
              ),
              tightPointDelta: requireFiniteNumber(
                move.outcome.tightPointDelta,
                `Melo move ${index} tight-point delta`,
              ),
            },
          }),
      ...(move.sourceKey === undefined
        ? {}
        : { sourceKey: requireString(move.sourceKey, `Melo move ${index} source key`) }),
      ...(move.acceptedAt === undefined
        ? {}
        : {
            acceptedAt: canonicalInstant(move.acceptedAt, `Melo move ${index} accepted time`),
          }),
      ...(move.dismissedAt === undefined
        ? {}
        : {
            dismissedAt: canonicalInstant(move.dismissedAt, `Melo move ${index} dismissed time`),
          }),
      ...(move.baselinePathSpare === undefined
        ? {}
        : {
            baselinePathSpare: requireFiniteNumber(
              move.baselinePathSpare,
              `Melo move ${index} baseline path spare`,
            ),
          }),
      ...(move.baselineTightPoint === undefined
        ? {}
        : {
            baselineTightPoint: requireFiniteNumber(
              move.baselineTightPoint,
              `Melo move ${index} baseline tight point`,
            ),
          }),
    })),
    ...
    meloMemoryThread: (state.meloMemoryThread ?? []).map((line, index) => ({
      id: requireString(line.id, `Melo memory ${index} ID`),
      at: canonicalInstant(line.at, `Melo memory ${index} time`),
      kind: line.kind,
      text: requireString(line.text, `Melo memory ${index} text`),
      editable: requireBoolean(line.editable, `Melo memory ${index} editability`),
      source: line.source,
    })),
    meloForgottenMemoryIds: checkedStringList(
      state.meloForgottenMemoryIds ?? [],
      'Forgotten Melo memory ID',
    ),
