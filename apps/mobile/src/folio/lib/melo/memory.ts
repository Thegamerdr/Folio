...
export type MemoryLineKind = 'moment' | 'whisper' | 'postcard' | 'move' | 'preference' | 'cadence';

export type MemoryLine = Readonly<{
  id: string;
  at: string;
  kind: MemoryLineKind;
  text: string;
...
/** Convert provable historical events into persisted, user-inspectable memory lines. */
export function observedMemoryLines(
  tinyWins: readonly TinyWin[],
  cycles: readonly CycleRecord[],
): MemoryLine[] {
  return deriveMeloMemory(tinyWins, cycles, Number.MAX_SAFE_INTEGER).map((event) => ({
    id: event.id,
    at: event.at,
    kind: event.kind === 'win' ? 'moment' : 'cadence',
    text: event.line,
    editable: true,
    source: 'observed',
  }));
