...
//
// WHY THIS EXISTS (MONEY_MODEL.md §2b, Free/Full/Live): AI statement reads are the app's only
...
// Free gets a small monthly allowance, Full a bigger one, Live unlimited (it's the tier whose
...
/** The monthly allowance for a tier — `null` means unlimited (Live). */
export function allowanceFor(tier: ReadTier): number | null {
  return tier === 'live' ? null : READ_ALLOWANCE[tier];
}
...
/** Reads remaining this month — `null` means unlimited (Live). Never negative. */
