...
describe('allowanceFor', () => {
  it('gives Free and Full their finite monthly allowances', () => {
    expect(allowanceFor('free')).toBe(READ_ALLOWANCE.free);
    expect(allowanceFor('full')).toBe(READ_ALLOWANCE.full);
  });

  it('gives Live unlimited (null)', () => {
    expect(allowanceFor('live')).toBeNull();
  });

  it('Full allows strictly more than Free (quantity is the tier difference, never quality)', () => {
    expect(READ_ALLOWANCE.full).toBeGreaterThan(READ_ALLOWANCE.free);
  });
});
...
describe('readsUsed / readsLeft / canReadNow — lazy monthly reset', () => {
  ...
  it('never blocks Live — unlimited means unlimited', () => {
    const state = { monthKey: JULY, used: 10_000 };
    expect(readsLeft(state, 'live', JULY)).toBeNull();
    expect(canReadNow(state, 'live', JULY)).toBe(true);
  });
