...
export type CanonicalAppStateMoneyProjection = Readonly<{
  ...
  meloPrimerSeenAt: NonNullable<AppState['meloPrimerSeenAt']> | null;
  lastOpenedAt: NonNullable<AppState['lastOpenedAt']> | null;
  oneMoveHistory: NonNullable<AppState['oneMoveHistory']>;
  meloMoves: NonNullable<AppState['meloMoves']>;
  meloDismissLog: NonNullable<AppState['meloDismissLog']>;
  meloMemoryThread: NonNullable<AppState['meloMemoryThread']>;
  meloForgottenMemoryIds: NonNullable<AppState['meloForgottenMemoryIds']>;
...
export function readCanonicalAppStateMoneyProjection(
  ...
  return {
    ...
    meloPrimerBeat: companionRuntime.meloPrimerBeat ?? 0,
    meloPrimerSeenAt: companionRuntime.meloPrimerSeenAt ?? null,
    lastOpenedAt: companionRuntime.lastOpenedAt ?? null,
    oneMoveHistory: (companionRuntime.oneMoveHistory ?? []).map((entry) => ({ ...entry })),
    meloMoves: (companionRuntime.meloMoves ?? []).map((move) => ({
      ...move,
      ...(move.outcome === undefined ? {} : { outcome: { ...move.outcome } }),
    })),
    meloDismissLog: (companionRuntime.meloDismissLog ?? []).map((entry) => ({ ...entry })),
    meloMemoryThread: (companionRuntime.meloMemoryThread ?? []).map((line) => ({ ...line })),
    meloForgottenMemoryIds: [...(companionRuntime.meloForgottenMemoryIds ?? [])],
...
function normalizedSourceMoneyProjection(
  ...
  return {
    ...
    meloPrimerBeat: state.meloPrimerBeat ?? 0,
    meloPrimerSeenAt: state.meloPrimerSeenAt ?? null,
    lastOpenedAt: state.lastOpenedAt ?? null,
    oneMoveHistory: (state.oneMoveHistory ?? []).map((entry) => ({ ...entry })),
    meloMoves: (state.meloMoves ?? []).map((move) => ({
      ...move,
      ...(move.outcome === undefined ? {} : { outcome: { ...move.outcome } }),
    })),
    meloDismissLog: (state.meloDismissLog ?? []).map((entry) => ({ ...entry })),
    meloMemoryThread: (state.meloMemoryThread ?? []).map((line) => ({ ...line })),
