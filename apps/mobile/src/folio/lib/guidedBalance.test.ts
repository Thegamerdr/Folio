...
describe('guided check-in production balance', () => {
  it('never substitutes demo cash for a fresh zero balance', () => {
    expect(guidedBalanceDraft({ amount: 0, source: 'user-entered' })).toBe('');
    expect(guidedBalanceDraft({ amount: 0, source: 'user-entered' }, true)).toBe('0');
  });
  it('does not present legacy sample cash for confirmation as real money', () => {
    expect(guidedBalanceDraft({ amount: 1240, source: 'sample' })).toBe('');
  });
  ...
  it('does not invent cash from an invalid stored amount', () => {
    expect(guidedBalanceDraft({ amount: Number.NaN, source: 'user-entered' })).toBe('');
  });
});
