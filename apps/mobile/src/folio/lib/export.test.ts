...
import { createMeloStreakState, createPhoenixStageState } from './melo/stage';
...
function fullState(): AppState {
  const stageAt = '2026-06-27T00:00:00.000Z';
  return {
    schemaVersion: 2,
    ...createPersonalWorkspaceRoot(),
    stage: createPhoenixStageState(stageAt),
    streak: createMeloStreakState(stageAt),
    pots: [
      {
        id: 'holiday',
...
describe('buildExport — per-surface csvs', () => {
  it('always emits the named per-surface CSV files', () => {
    ...
    expect(csvs).toHaveProperty('decision-ledger.csv');
  });
