...
export function MeloLine({ text, mood = 'calm', size = 28, asleep = false }: MeloLineProps) {
  const t = useTheme();
  const showCharacter = useMeloInlineCharacterVisible();

  return (
    <View style={styles.row}>
      {showCharacter ? <Melo mood={mood} size={size} asleep={asleep} /> : null}
