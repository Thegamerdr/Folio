...
export function GuidedCheckInScreen({ nav, state = 'populated' }: GuidedCheckInScreenProps) {
  ...
  // Seed only from a real account position. A clean account stays blank: a prototype amount must
  // never look like the user's money.
  const seededAmount = useAppStore((s) => s.currentBalance.amount);
  const seededSource = useAppStore((s) => s.currentBalance.source);
  const [value, setValue] = useState(() =>
    seededAmount > 0 && seededSource !== 'sample' ? String(Math.round(seededAmount)) : '',
  );
  ...
  const shownNumber = hasValue ? Number(value) : 0;
      ...
      >
        ...
        <View style={[styles.card, { backgroundColor: t.surface, borderColor: t.hairline }]}>
          ...
          <View style={styles.amountRow}>
            ...
            <Text
              accessibilityLabel={
                hasValue ? `${copy.global.currency.symbol}${grouped}` : 'No balance entered'
              }
              style={[styles.amount, { color: t.ink }]}
            >
              {hasValue ? countedLabel : ''}
            </Text>
        ...
        <Pressable
          ...
          style={({ pressed: isPressed }) => [
            styles.continue,
            { backgroundColor: t.calm, opacity: hasValue ? 1 : 0.4 },
            isPressed && hasValue ? styles.pressed : undefined,
          ]}
        >
          <Text style={[styles.continueLabel, { color: t.accentInk }]}>Continue</Text>
        </Pressable>
      </ScrollView>
    </Animated.View>
  );
