...
const recovery = read('./RecoveryScreen.tsx').replace(/\r\n/g, '\n');
const pdfSuccess = read('./PdfSuccessScreen.tsx');
const guidedCheckIn = read('./GuidedCheckInScreen.tsx');
const chartStyle = read('../sheets/ChartStyleSheet.tsx');
const businessToday = read('./BusinessTodayScreen.tsx');
...
describe('no fabricated content on real-user screens', () => {
  ...
  it('BusinessTodayScreen does not silently materialise recurring invoice drafts', () => {
    expect(businessToday).not.toContain('generateDueRecurringInvoices');
    expect(businessToday).not.toContain('updateBusinessOperations(generated)');
  });

  it('clean-state check-in and chart previews contain no example money', () => {
    expect(guidedCheckIn).not.toContain("SEED_FALLBACK = '1240'");
    expect(guidedCheckIn).toContain("seededSource !== 'sample'");
    expect(chartStyle).not.toContain('£1,240');
    expect(chartStyle).not.toContain('£340');
    expect(chartStyle).not.toContain('£2,180');
  });
});
