...
export function BusinessRunwayScreen({ nav }: { nav: Nav }) {
  ...
  const nextMovements = runway.forecast.filter((point) => point.deltaMinor !== 0).slice(0, 8);
  const headline =
    accounts.length === 0
  ...
  return (
    <>
      <BusinessScreenFrame
        eyebrow="Cash runway"
        headline={headline}
        intro={`Built from ${formatMinor(runway.cashMinor)} in non-card accounts, dated invoices and recurring money out.`}
        onBack={nav.back}
      >
        <BusinessCard tone="inset">
          <View style={styles.metrics}>
            <BusinessMetric label="Cash in hand" value={formatMinor(runway.cashMinor)} />
            <BusinessMetric label="Due in · 30d" value={formatMinor(runway.incoming30Minor)} />
            <BusinessMetric label="Due out · 30d" value={formatMinor(runway.outgoing30Minor)} />
          </View>
        </BusinessCard>
        ...
        <View style={styles.section}>
          ...
          <BusinessCard>
            {accounts.length === 0 ? (
              ...
              <Text style={[styles.emptyBody, { color: t.muted }]}>
                No dated invoice or recurring cost changes the next 90 days yet.
              </Text>
            ) : (
              nextMovements.map((movement, index) => (
                <View
                  key={movement.date}
                  style={[
                    styles.movement,
                ...
                >
                  <View style={styles.movementCopy}>
                    <Text style={[styles.movementDate, { color: t.ink }]}>
                      {formatBusinessDate(movement.date)}
                    </Text>
                    <Text style={[styles.movementBalance, { color: t.muted }]}>
                      {formatMinor(movement.balanceMinor)} after
                    </Text>
                  </View>
                  <Text
                    style={[
                      styles.movementAmount,
                      { color: movement.deltaMinor >= 0 ? t.calmStrong : t.ink },
                    ]}
                  >
                    {formatMinor(movement.deltaMinor, { signed: true })}
                  </Text>
                </View>
              ))
            )}
        ...
        <View style={styles.routes}>
          <BusinessRouteRow
            hint="Outstanding and overdue"
            label="Who owes you"
            onPress={() => nav.go('business-invoices')}
            value={formatMinor(totalOutstandingInvoicesMinor(business))}
          />
          <BusinessRouteRow
            hint="Rent, software, tax and other repeats"
            label="What is due out"
        ...
        {accounts.length === 0 ? (
          ...
          <BusinessPrimaryAction
            label="Move owner money"
            onPress={() => {
              setOwnerError('');
              setMovingOwnerMoney(true);
            }}
          />
        )}
      </BusinessScreenFrame>
...
type InvoiceCadence = 'one-off' | 'weekly' | 'fortnightly' | 'monthly' | 'quarterly' | 'annual';
...
export function BusinessInvoicesScreen({ nav }: { nav: Nav }) {
  ...
  const [cadence, setCadence] = useState<InvoiceCadence>('one-off');
  ...
  const save = () => {
    ...
    const invoice: BusinessInvoice = {
      ...
      paidMinor: 0,
      status: 'issued',
      ...(cadence !== 'one-off' ? { recurringTemplateId: `recurring-${Date.now()}` } : {}),
    };
    updateBusinessOperations((state) => ({
      ...
      recurringInvoices:
        cadence === 'one-off'
          ...
          : [
              ...state.recurringInvoices,
              {
                id: invoice.recurringTemplateId!,
                clientId: client.id,
                clientName: client.name,
                amountMinor: totalMinor,
                ...(reference.trim() ? { reference: reference.trim() } : {}),
                cadence,
                nextIssueOn: nextCadenceDate(invoice.issuedOn, cadence),
                daysToPay: Math.max(
                  0,
                  Math.round(
                  ...
                  {row.invoices.map((invoice, index) => (
                    ...
                    >
                      <View style={styles.invoiceCopy}>
                        ...
                        {row.bucket !== 'current' ? (
                          <Text style={[styles.invoiceMeta, { color: t.calmStrong }]}>
                            Eligible B2B claim working figure{' '}
                            {formatMinor(
                              calculateLateCommercialPaymentClaim(invoice).totalClaimMinor,
                            )}{' '}
                            · 11.75% statutory interest +{' '}
                            {formatMinor(
                              calculateLateCommercialPaymentClaim(invoice).fixedCompensationMinor,
                            )}{' '}
                            fixed compensation
                          </Text>
                        ) : null}
...
export function BusinessVatScreen({ nav }: { nav: Nav }) {
  ...
  const [potTopUp, setPotTopUp] = useState('');
  ...
  const addToPot = () => {
    const amountMinor = parseMinor(potTopUp);
    if (amountMinor === null || amountMinor <= 0) return;
    ...
    <>
      <BusinessScreenFrame
        ...
        headline={
          boxes
            ? boxes.box5Minor >= 0
              ? `${formatMinor(boxes.box5Minor)} to set aside.`
      ...
      >
        <BusinessCard tone="inset">
            ...
            <View style={styles.cardHeadingCopy}>
              ...
              <Text style={[styles.cardMeta, { color: t.muted }]}>
                {liability > business.vatPotMinor
                  ? `${formatMinor(liability - business.vatPotMinor)} to cover the current net amount`
                  : 'Current net amount covered'}
              </Text>
            </View>
          ...
          <View style={styles.inlineForm}>
            ...
            <Pressable
              accessibilityRole="button"
              disabled={(parseMinor(potTopUp) ?? 0) <= 0}
              onPress={addToPot}
              style={({ pressed }) => [
                styles.inlineAction,
                {
                  backgroundColor: t.calmStrong,
                  opacity: (parseMinor(potTopUp) ?? 0) <= 0 ? 0.38 : pressed ? 0.68 : 1,
                },
              ]}
            >
              <Text style={[styles.inlineActionLabel, { color: t.inverse }]}>Add</Text>
            </Pressable>
          </View>
        </BusinessCard>

        <View style={styles.section}>
          <BusinessSectionTitle title="Accounting method" value={schemeLabel(currentVat?.scheme)} />
          <BusinessCard>
            <View style={styles.cardHeading}>
              <View style={styles.cardHeadingCopy}>
                <Text style={[styles.cardTitle, { color: t.ink }]}>Which scheme fits?</Text>
                <Text style={[styles.cardMeta, { color: t.muted }]}>
                  {schemeAnalysis
                    ? `Compared from ${recentReturns.length} recorded return${
                        recentReturns.length === 1 ? '' : 's'
                      } in the last 12 months.`
                    : 'The comparison starts after a return has real VAT boxes.'}
                </Text>
              </View>
              {schemeAnalysis ? (
                <Text style={[styles.schemeTag, { color: t.calmStrong }]}>
                  {schemeLabel(schemeAnalysis.recommendation)}
                </Text>
              ) : null}
            </View>
          </BusinessCard>
          <BusinessSecondaryAction
            label="Compare VAT schemes"
            onPress={() => {
              setSchemeChoice(currentVat?.scheme ?? 'standard');
              setSchemeOpen(true);
            }}
          />
        </View>
        ...
        <View style={styles.section}>
          {openReturn && boxes ? (
            <>
              <BusinessSectionTitle
                title={`${formatBusinessDate(openReturn.periodStart)} – ${formatBusinessDate(openReturn.periodEnd)}`}
                value={`Due ${formatBusinessDate(openReturn.dueOn)}`}
              />
              <BusinessCard>
                {(
                  [
                    ['1', 'Output VAT', boxes.box1Minor],
                    ['2', 'NI acquisitions VAT', boxes.box2Minor],
                    ['3', 'Total VAT due', boxes.box3Minor],
                    ['4', 'Input VAT', boxes.box4Minor],
                    ['5', 'Net VAT', boxes.box5Minor],
                    ['6', 'Sales excluding VAT', boxes.box6Minor],
                    ['7', 'Purchases excluding VAT', boxes.box7Minor],
                    ['8', 'NI EU goods sales', boxes.box8Minor],
                    ['9', 'NI EU goods purchases', boxes.box9Minor],
                  ] as const
              ...
              <BusinessPrimaryAction
                label="Prepare VAT working copy"
                onPress={() => nav.go('business-filing-vat')}
              />
            </>
          ) : (
            <BusinessCard tone="inset">
              <Text style={[styles.emptyTitle, { color: t.ink }]}>Nothing open.</Text>
              <Text style={[styles.emptyBody, { color: t.muted }]}>
                Add the period and four source boxes when the real quarter is ready.
              </Text>
            </BusinessCard>
          )}
        </View>
        {!openReturn ? (
          <BusinessPrimaryAction label="Start a VAT return" onPress={() => setAdding(true)} />
        ) : null}
      </BusinessScreenFrame>
      <BusinessFormSheet
...
export function BusinessInsightsScreen({ nav }: { nav: Nav }) {
  ...
  const paidRevenue = business.invoices
    .filter((invoice) => invoice.status === 'paid')
    .reduce((sum, invoice) => sum + invoice.paidMinor, 0);
  const transactionRevenue = transactions
    .filter((transaction) => transaction.amount > 0)
    .reduce((sum, transaction) => sum + Math.round(transaction.amount * 100), 0);
  const transactionCosts = Math.abs(
    transactions
      .filter((transaction) => transaction.amount < 0)
      .reduce((sum, transaction) => sum + Math.round(transaction.amount * 100), 0),
  );
  const revenue = Math.max(paidRevenue, transactionRevenue);
  const net = revenue - transactionCosts;
  const byClient = business.clients
    .map((client) => ({
      name: client.name,
      totalMinor: business.invoices
  ...
  return (
    <BusinessScreenFrame
      ...
      headline={
        hasShape
          ? net >= 0
            ? 'The year is above water.'
            : 'Costs are ahead of revenue.'
          : 'Not enough shape yet.'
      }
      intro="A calm look at what came in, what went out and who the work came from."
      onBack={nav.back}
    >
      <BusinessCard tone="inset">
        <View style={styles.metrics}>
          <BusinessMetric label="In" value={formatMinor(revenue)} />
          <BusinessMetric label="Out" value={formatMinor(transactionCosts)} />
          <BusinessMetric accent={net >= 0} label="Net" value={formatMinor(net)} />
        </View>
      </BusinessCard>

      <View style={styles.section}>
        <BusinessSectionTitle title="Top clients" value="By invoiced value" />
        <BusinessCard>
          {byClient.length === 0 ? (
            <Text style={[styles.emptyBody, { color: t.muted }]}>
              Clients appear after a real invoice is saved.
            </Text>
          ) : (
            byClient.map((client, index) => (
              ...
              >
                <Text style={[styles.simpleTitle, { color: t.ink }]}>{client.name}</Text>
                <Text style={[styles.simpleMoney, { color: t.ink }]}>
                  {formatMinor(client.totalMinor)}
                </Text>
              </View>
            ))
          )}
      ...
      <View style={styles.routes}>
        <BusinessRouteRow
          hint="Mileage, home office, pension and CIS"
          label="Deductions"
          onPress={() => nav.go('business-deductions')}
        />
        <BusinessRouteRow
          hint="Prepared returns and external-submission records"
          label="Filings"
          onPress={() => nav.go('business-filings')}
        />
      </View>
    </BusinessScreenFrame>
  );
...
function nextCadenceDate(date: string, cadence: Exclude<InvoiceCadence, 'one-off'>): string {
  const value = new Date(`${date}T00:00:00Z`);
  if (cadence === 'weekly') value.setUTCDate(value.getUTCDate() + 7);
  else if (cadence === 'fortnightly') value.setUTCDate(value.getUTCDate() + 14);
...
const styles = StyleSheet.create({
  ...
  movementCopy: { flex: 1 },
  movementDate: { fontSize: 13, fontWeight: '600' },
  movementBalance: { fontSize: 11.5, marginTop: 2 },
  movementAmount: { fontSize: 13.5, fontVariant: ['tabular-nums'], fontWeight: '600' },
