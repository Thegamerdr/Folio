  ...
  return (
    <View style={[ui.root, { backgroundColor: t.canvas }]}>
      ...
      >
        ...
        <MeloCompanionExclusion id="business-frame/body" attentionSalience={0.9} style={ui.body}>
          {children}
        </MeloCompanionExclusion>
      </ScrollView>
    </View>
...
export function BusinessPrimaryAction({
  ...
  return (
    ...
    >
      <Text style={[ui.primaryLabel, { color: t.canvas }]}>{label}</Text>
    </Pressable>
  );
}
      ...
      <View style={ui.pills}>
        {options.map((option) => {
          ...
          return (
            ...
            >
              <Text style={[ui.pillLabel, { color: selected ? t.accentInk : t.ink }]}>
                {option.label}
              </Text>
            </Pressable>
          );
