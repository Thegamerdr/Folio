...
//
// @rn-engine ocr-extraction (bundled ML Kit attempted locally before this fallback)
...
//   Text Recognition did not produce reliable rows). The image was saved to the app cache
...
//   candidates and route to image-success instead. The card renders the retained image's real name.
//
  ...
  if (state === 'empty') {
    return (
      <StatePanel
        ...
        fullScreen
        kind="genuine-empty"
        primaryAction={{ label: 'Try another image', onPress: () => nav.go('intake') }}
  ...
  return (
    <Animated.View style={[styles.root, enterStyle, { backgroundColor: t.canvas }]}>
      ...
      >
        ...
        <IntakeResultRail nav={nav} outcome="needs-help" source="image" />
        ...
        >
          <Text style={[styles.primaryLabel, { color: t.accentInk }]}>Try another image</Text>
        </Pressable>
