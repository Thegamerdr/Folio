...
import { FirstAnswerScreen } from '@/folio/screens/FirstAnswerScreen';
...
import { IntakeHistoryScreen } from '@/folio/screens/IntakeHistoryScreen';
...
import { CompanionTouchesSheet } from '@/folio/sheets/CompanionTouchesSheet';
...
import { EarnStamp } from '@/folio/ui/EarnStamp';
...
import { deriveMeloState } from '@/folio/lib/melo/state';
import { weatherTintCanvas } from '@/folio/lib/melo/weatherTint';
...
function ReadyFolioShell() {
  ...
  const meloQuietMode = useAppStore((st) => st.melo?.quietMode === true);
  const meloWardrobe = useAppStore((st) => st.melo?.wardrobe ?? []);
  const ambientSubs = useAppStore((st) => st.subs);
  const ambientSubPaused = useAppStore((st) => st.subPaused);
  const ambientPots = useAppStore((st) => st.pots);
  const ambientBalance = useAppStore((st) => st.currentBalance);
  const ambientOnboarding = useAppStore((st) => st.onboarding);
  ...
  const ambientWeather = useMemo(
    () =>
      deriveMeloState({
        tightestSpare: pressureRoute.tightPoint.amount,
        monthlyIncome: Math.max(1, ambientOnboarding.monthlyIncome),
        subs: ambientSubs,
        subPaused: ambientSubPaused,
        pots: ambientPots,
        currentBalance: ambientBalance,
        onboarding: ambientOnboarding,
      }).weather,
    [
      ambientBalance,
      ambientOnboarding,
      ...
      ambientSubPaused,
      ambientSubs,
      pressureRoute.tightPoint.amount,
    ],
  );
  const ambientCanvas = weatherTintCanvas(t.canvas, ambientWeather, {
    quiet: meloQuietMode,
  });

  const refreshWorkspaceStage = useCallback((now: Date) => {
    const snapshot = getState();
    const workspace = snapshot.workspaces.find(
      (candidate) => candidate.id === snapshot.activeWorkspaceId,
    );
    if (workspace?.kind === 'business') {
      refreshBusinessMeloProgress(now);
      return;
    }

    const route = routeFromStore(snapshot, now);
    const latestLivedCycle = snapshot.cycles.find((cycle) => cycle.reconstructed !== true) ?? null;
    const headroom = Math.max(1, Math.abs(snapshot.currentBalance.amount));
    const hoursSincePayday =
      snapshot.stage.lastRebirthAt === null
        ? Number.POSITIVE_INFINITY
        : Math.max(
            0,
            (now.getTime() - Date.parse(snapshot.stage.lastRebirthAt)) / (60 * 60 * 1000),
          );
    refreshMeloStage(
      {
        ...
        cycleLength: 30,
        pathBendPct:
          route.tightPoint.amount < 0
            ? Math.min(1, Math.abs(route.tightPoint.amount) / headroom)
            : 0,
        cleanStreakDays: snapshot.streak.count,
        lastCycleRed:
          latestLivedCycle !== null &&
          (latestLivedCycle.spare < 0 || latestLivedCycle.tightPoint < 0),
        hoursSincePayday,
        mode:
          snapshot.moneyMode === 'survival'
            ? 'survival'
            : snapshot.moneyMode === 'reset'
              ? 'reset'
              : snapshot.moneyMode === 'lowVis'
                ? 'lowvis'
                : snapshot.moneyMode === 'irregular'
                  ? 'irregular'
                  : 'standard',
      },
      now,
  ...
  const nav = useMemo<Nav>(
    () => ({
      go,
      back,
      openWorkspace: openWorkspaceSheet,
      openSheet,
      openMelo,
  ...
  useEffect(() => {
    reanchorSubRenewals();
    refreshWorkspaceStage(new Date());
  ...
  return (
    ...
    <UndoProvider>
      <SheetPortalProvider>
        <MeloCompanionHost
          modalOpen={sheet !== null || workspaceSheetVisible}
          onOpenMelo={openMelo}
          quiet={meloQuietMode}
          screen={screen}
          wardrobe={meloWardrobe}
        >
          ...
          This grouped host gives Android one complete frame to paint, without a page-wide animation. */}
            <View
              ...
              style={[shellStyles.routeFrame, { backgroundColor: ambientCanvas }]}
            >
              <View
                collapsable={false}
                style={[shellStyles.screenHost, { backgroundColor: ambientCanvas }]}
              >
                <ScreenErrorBoundary
                  key={`screen-${screen}`}
                  screenLabel={screen}
            ...
            <View
              pointerEvents="none"
              style={[
                shellStyles.statusBarMask,
                { height: insets.top + 8, backgroundColor: ambientCanvas },
              ]}
            />
          ...
          recurring bill/debt-payment quick-add) — see the SheetId union's doc-comment in types.ts. */}
            ...
            {sheet === 'debt-schedule' && <DebtScheduleSheet visible onClose={closeSheet} />}
          ...
          (prefill/seed) so an "Ask Melo" CTA opens the chat with its draft. */}
            ...
            {sheet === 'companion-touches' && (
              <CompanionTouchesSheet visible onClose={closeSheet} />
            )}
...
function HydrationNotice() {
  ...
  return (
    ...
    >
      ...
      >
        <Text style={[noticeStyles.dismissLabel, { color: t.calmStrong }]}>OK</Text>
      </Pressable>
    </View>
  );
}
...
function PersistenceSaveNotice() {
  ...
  return (
    ...
    >
      ...
      >
        <Text style={[noticeStyles.dismissLabel, { color: t.calmStrong }]}>Try again</Text>
      </Pressable>
    </View>
  );
}
...
function ScreenView({ screen, nav, pressure }: { screen: ScreenId; nav: Nav; pressure: Pressure }) {
  ...
  if (screen === 'first-answer') return <FirstAnswerScreen nav={nav} />;
  ...
  if (screen === 'melo-memory') return <MeloMemoryScreen nav={nav} />;
  if (screen === 'melo-moves') return <MeloMovesScreen nav={nav} />;
  ...
  if (screen === 'intake-history') return <IntakeHistoryScreen nav={nav} />;
...
const SHEET_TITLE: Readonly<Record<NonNullable<SheetId>, string>> = {
  ...
  'edit-item': 'Edit',
  'melo-chat': 'Melo',
  'companion-touches': 'Companion touches',
  share: 'Share',
  onboarding: 'Welcome',
