 ...
 */
export function parseLocalMoneySuggestion(
  prompt: string,
  workspaceKind: 'personal' | 'business' = 'personal',
): ParsedSuggestion | null {
  const text = prompt.trim();
  if (text.length === 0 || HYPOTHETICAL.test(text)) return null;

  if (workspaceKind === 'business') {
    const sentAmountFirst = text.match(
      new RegExp(
        `\\b(?:sent|issued)\\s+(?:an?\\s+)?invoice\\s+for\\s+${MONEY}\\s+to\\s+(.+?)\\s+due\\s+(\\d{4}-\\d{2}-\\d{2})(?:[.!?]|$)`,
        'i',
      ),
    );
    const sentClientFirst = text.match(
      new RegExp(
        `\\b(?:sent|issued)\\s+(.+?)\\s+(?:an?\\s+)?invoice\\s+for\\s+${MONEY}\\s+due\\s+(\\d{4}-\\d{2}-\\d{2})(?:[.!?]|$)`,
        'i',
      ),
    );
    if (sentAmountFirst || sentClientFirst) {
      const amount = amountOf(sentAmountFirst?.[1] ?? sentClientFirst?.[2] ?? '');
      const client = cleanName(sentAmountFirst?.[2] ?? sentClientFirst?.[1], 'client');
      const dueOn = String(sentAmountFirst?.[3] ?? sentClientFirst?.[3] ?? '').trim();
      if (amount === null || !dueOn) return null;
      return {
        name: 'log_invoice_sent',
        args: { amount, client, dueOn },
        summary: `Record the £${amount.toFixed(2)} invoice sent to ${client}.`,
      };
    }

    const paidByReference = text.match(
      new RegExp(`\\binvoice\\s+(.+?)\\s+(?:was\\s+)?paid(?:\\s+${MONEY})?(?:[.!?]|$)`, 'i'),
    );
    if (paidByReference) {
      const target = cleanName(paidByReference[1], 'invoice');
      const amount = paidByReference[2] ? amountOf(paidByReference[2]) : null;
      return {
        name: 'log_invoice_paid',
        args: { invoice: target, ...(amount ? { amount } : {}) },
        summary: `Record payment for ${target}.`,
      };
    }

    const dividend = text.match(
      new RegExp(
        `\\b(?:declared|paid)\\s+(?:a\\s+)?${MONEY}\\s+dividend(?:\\s+(?:to|for)\\s+(.+?))?(?:[.!?]|$)`,
        'i',
      ),
    );
    if (dividend) {
      const amount = amountOf(dividend[1] ?? '');
      if (amount === null) return null;
      const shareholder = cleanName(dividend[2], '');
      return {
        name: 'log_dividend',
        args: { amount, ...(shareholder ? { shareholder } : {}) },
        summary: `Record the £${amount.toFixed(2)} dividend.`,
      };
    }

    const ownerDraw = text.match(
      new RegExp(
        `\\b(?:took|drew|withdrew)\\s+${MONEY}\\s+(?:out\\s+)?from\\s+(?:the\\s+)?(?:business|company)(?:[.!?]|$)`,
        'i',
      ),
    );
    if (ownerDraw) {
      const amount = amountOf(ownerDraw[1] ?? '');
      if (amount === null) return null;
      return {
        name: 'log_owner_draw',
        args: { amount },
        summary: `Record the £${amount.toFixed(2)} owner draw.`,
      };
    }

    const income = text.match(
      new RegExp(
        `\\b(?:business\\s+)?(?:received|earned|got\\s+paid)\\s+${MONEY}(?:\\s+from\\s+(.+?))?(?:[.!?]|$)`,
        'i',
      ),
    );
    if (income) {
      ...
      if (amount === null) return null;
      const source = cleanName(income[2], 'business income');
      return {
        name: 'log_business_income',
        args: { amount, source },
        summary: `Log £${amount.toFixed(2)} received from ${source}.`,
      };
    }

    const expense = text.match(
      new RegExp(
        `\\b(?:business\\s+)?(?:spent|paid)\\s+${MONEY}(?:\\s+(?:at|to|on)\\s+(.+?))?(?:[.!?]|$)`,
        'i',
      ),
    );
    if (expense) {
      const amount = amountOf(expense[1] ?? '');
      if (amount === null) return null;
      const merchant = cleanName(expense[2], 'business expense');
      return {
        name: 'log_business_expense',
        args: { amount, merchant, category: categoryFor(merchant) },
        summary: `Log £${amount.toFixed(2)} paid to ${merchant}.`,
      };
    }
    return null;
  }
  ...
  const borrow = text.match(
    new RegExp(`\\b(?:borrowed|took)\\s+${MONEY}\\s+from\\s+(.+?)(?:\\s+pot)?(?:[.!?]|$)`, 'i'),
  );
  if (borrow) {
    const amount = amountOf(borrow[1] ?? '');
    if (amount === null) return null;
    const pot = cleanName(borrow[2], 'pot').replace(/\s+pot$/i, '');
    return {
      name: 'borrowFromPot',
      args: { amount, pot },
      summary: `Borrow £${amount.toFixed(2)} from ${pot}.`,
    };
  }

  const topUp = text.match(
    new RegExp(
      `\\b(?:added|put)\\s+${MONEY}\\s+(?:into|in|to)\\s+(.+?)(?:\\s+pot)?(?:[.!?]|$)`,
      'i',
    ),
  );
  if (topUp) {
    const amount = amountOf(topUp[1] ?? '');
    if (amount === null) return null;
    const pot = cleanName(topUp[2], 'pot').replace(/\s+pot$/i, '');
    return {
      name: 'addToPot',
      args: { amount, pot },
      summary: `Add £${amount.toFixed(2)} to ${pot}.`,
    };
  }
...
export function buildLocalMeloTurn(
  ...
  const parsed = parseLocalMoneySuggestion(
    input.prompt,
    input.snapshot.workspaceKind ?? 'personal',
  );
  if (parsed !== null) {
    return {
      reply: withTone(
