...
export function acceptLocalMeloRephrase(
  ...
  if (
    /```|<\|[^>]+\|>|https?:\/\/|www\.|\b(?:log_spend|log_income|log_refund|log_transfer|addToPot|borrowFromPot|log_business_expense|log_business_income|log_invoice_sent|log_invoice_paid|log_owner_draw|log_dividend)\b/i.test(
      candidate,
    )
  ) {
    return null;
