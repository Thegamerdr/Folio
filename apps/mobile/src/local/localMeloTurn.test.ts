...
describe('local Melo turn', () => {
  ...
  it('parses the two Personal pot tools as review-only proposals', () => {
    expect(parseLocalMoneySuggestion('I put £25 into Holiday pot')).toMatchObject({
      name: 'addToPot',
      args: { amount: 25, pot: 'Holiday' },
    });
    expect(parseLocalMoneySuggestion('I borrowed £10 from Buffer pot')).toMatchObject({
      name: 'borrowFromPot',
      args: { amount: 10, pot: 'Buffer' },
    });
  });

  it('uses the Business tool family only in a Business workspace', () => {
    const businessSnapshot = { ...snapshot, workspaceKind: 'business' as const };
    expect(
      buildLocalMeloTurn({
        prompt: 'The business paid £48 to Figma',
        snapshot: businessSnapshot,
        tone: 'calm',
      }).suggestions[0],
    ).toMatchObject({
      name: 'log_business_expense',
      args: { amount: 48, merchant: 'Figma' },
    });
    expect(
      parseLocalMoneySuggestion('I sent an invoice for £500 to Acme due 2026-08-31', 'business'),
    ).toMatchObject({
      name: 'log_invoice_sent',
      args: { amount: 500, client: 'Acme', dueOn: '2026-08-31' },
    });
    expect(
      parseLocalMoneySuggestion('I declared a £200 dividend to Avery', 'business'),
    ).toMatchObject({
      name: 'log_dividend',
      args: { amount: 200, shareholder: 'Avery' },
    });
  });
});
