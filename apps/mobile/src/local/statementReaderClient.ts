...
const SYSTEM_PROMPT = [
  ...
  '{ "items": [ { "date": "YYYY-MM-DD" | null, "merchant": string, "amount": number, "category": string | null } ], "openingBalance": number | null, "closingBalance": number | null, "closingDate": "YYYY-MM-DD" | null, "statedTotalDebits": number | null, "statedTotalCredits": number | null }',
  'Rules for each item:',
  ...
  'inside "items".',
  'Separately, at the top level (these let us check the extracted rows add up — return them when the',
  'statement clearly prints them, else null; NEVER guess or compute them yourself):',
  '- "openingBalance": the statement\'s OPENING/START balance for the period, in pounds.',
  '- "closingBalance": the statement\'s CLOSING/END balance, in pounds, and "closingDate": the ISO',
  '  date that closing balance is as-of (usually the last day of the statement period).',
  '- "statedTotalDebits": the statement\'s own printed TOTAL of money out (all debits/payments added',
  '  up), as a POSITIVE number in pounds.',
  '- "statedTotalCredits": the statement\'s own printed TOTAL of money in (all credits/deposits added',
  '  up), as a POSITIVE number in pounds.',
