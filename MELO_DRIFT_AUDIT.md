...
### 🔴 Choosable/customizable mascot roster — _reinterpreted_
...
### 🟠 Cracked-heart / damage states — _partial_
...
### 🟠 Mascot care loop — _reinterpreted_
...
### 🟠 Mascot centrality vs. Quiet Mode (blueprint addition that hedges the owner's bet) — _reinterpreted_
...
### 🟠 Achievement cosmetics as sellable emotional premium — _reinterpreted_
...
The brief lists 'achievement cosmetics' as a PREMIUM (sellable) category. The blueprint inverts it: achievement items are 'earned only, never sold... visibly _unbuyable_ — that's what makes them status' (§3.5). Similarly 'rare outfits' is neutered — §8.1.5 bans artificial scarcity ('Seasonal items return every year', 'limited-time but not artificially scarce'), and 'animated reactions' as premium collides with §8.1.2 'Mood is never for sale' so only celebration-effect _styles_ are sellable. These are likely correct ethics calls, but three of the owner's nine named emotional-premium revenue lines were deleted or softened with no §0 entry flagging the change.

### 🟠 Functional premium: alerts and advanced recovery plans as paid features — _partial_
...
### ⚪ Brand packs — _dropped-silently_
...
### ⚪ Money Weather screen — _deferred-explicitly_
...
### ⚪ Money Time Machine as its own pillar and screen — _deferred-explicitly_
...
### ⚪ Premium tease in onboarding — _partial_
...
### ⚪ Voice/personality styles — _reinterpreted_
...
### ⚪ Requested state list fidelity (Storm, Low Balance) — _reinterpreted_
...
### ⚪ Everything else in the brief's 16 sections — _built_
...
### 🔴 Home-screen widget (the glance loop) — _dropped-silently_

**Source:** §14 MVP item 14: "One widget (small: mascot + number + weather tint)"; §5.1: "for many users the widget _is_ the app most days"; metric: "Widget adoption ≥25% of D7 users (the glance loop's leading indicator)"
...
### 🔴 Notification delivery (all of it) — _partial_
...
### 🟠 WINNING state (and everything trajectory-based) — _dropped-silently_
...
### 🟠 MILESTONE overlay + milestone notification + commemorative items — _dropped-silently_

**Source:** §4.2 MILESTONE: "buffer thresholds (£100/£500/£1k), debt-free date, N green cycles, 1-year anniversary… commemorative item appears in wardrobe (earned, unbuyable)… Notification: yes — this is what notifications are _for_"
...
### 🟠 NEGLECTED → RETURN (the welcome back) — _dropped-silently_
...
### 🟠 UNSAFE UNTIL PAYDAY (the honest shortfall flow) — _dropped-silently_
...
### 🟠 BNPL visibility — _dropped-silently_
...
### 🟠 Mascot fidelity: intensities, idle library, rig — _partial_
...
### 🟠 Onboarding beats 6–7 + resume + irregular income — _partial_
...
### 🟠 Afford-check asserts the forecast instead of computing it — _reinterpreted_
...
### 🟠 Payday ritual savings step is a non-choice — _partial_
...
### 🟠 Statement import: PDF path — _partial_
...
### ⚪ Danger-state 3-item micro-plan — _partial_
...
### ⚪ monetizationAllowed + overdraft embargo consumed nowhere — _partial_
...
### ⚪ Accuracy-feedback trust metric not instrumented — _partial_
...
### ⚪ Comfortable per-day threshold hardwired — _reinterpreted_
...
### ⚪ Tiny-win types diverge from the blueprint's list — _reinterpreted_
...
### ⚪ Hysteresis bands replaced with a different anti-flap scheme — _reinterpreted_
...
### ⚪ SAFE UNTIL PAYDAY banner (P80 confidence) — _dropped-silently_
...
### ⚪ Clear-night and snow weather — _deferred-explicitly_
...
### ⚪ Fog suspends some forecasting but not all — _partial_
...
### 🔴 Notifications v1 (delivery) — _partial_
...
### 🔴 Phase-1 user-validation gate (test kit) — no record it ran — _dropped-silently_
...
### 🟠 Widget (small: mascot + number + tint) — _deferred-explicitly_
...
### 🟠 Copy linter coverage (§10.3 'enforced as CI lint') — _partial_
...
### 🟠 Payday patterns + weekend-shift rule (§13 risk 16) — _dropped-silently_
...
### 🟠 Irregular / variable income path (§13 risk 8) — _dropped-silently_
...
### 🟠 BNPL visibility (§13 risk 11) — _partial_
...
### 🟠 Safe Zone setup beat (buffer + essentials questions) — _dropped-silently_
...
### 🟠 Mascot reacting live during onboarding — _dropped-silently_
...
### 🟠 Unsafe-cycle options flow (§4 UNSAFE, §13 risk 12) — _dropped-silently_
...
### 🟠 Accuracy-feedback instrumentation (trust flywheel) — _partial_
...
### 🟠 Money Weather forecast sheet (7-day) — _partial_
...
### 🟠 Payday Ritual — cycle card + lighter-income handling; sweep is presentational — _partial_
...
### 🟠 Mascot progression loop (earned items, milestones) — _dropped-silently_
...
### 🟠 Settings trust infrastructure: data export + delete — _dropped-silently_
...
### ⚪ Monetization / store / Plus (§8) — _deferred-explicitly_
...
### ⚪ Recovery extras: co-breathing invite + 'talk it through' (Ask Melo) — _partial_
...
### ⚪ Before You Spend — 'Safe on [date]' verdict + danger-date impact + example chips — _partial_
...
### ⚪ Social/share loop — _dropped-silently_
...
### ⚪ Weekly review — delivery + weather recap — _partial_
...
### ⚪ Neglected→Return welcome-back flow — _partial_
...
### ⚪ Savings goals / buffer tracker — _partial_
...
### ⚪ Overdraft-aware model ('my true floor') — _partial_
...
### ⚪ Milestone ladder + tiny-win celebrations — _partial_
