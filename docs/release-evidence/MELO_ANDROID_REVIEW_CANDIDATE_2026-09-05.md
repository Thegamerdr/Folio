...
The visible emulator preview was recovered after its development servers stalled during build work. The current normal preview is served on port **8082** to `com.folio.v2.greenfield.preview`; root inspected the rendered Today screen (`apps/mobile/.expo/emulator-root-live.png`). A capture deep link was ignored by this normal server, as expected. The attempted post-fix synthetic Calendar Go There interaction did not produce accepted proof and is **not verified** by this pass; the jump fix has source/no-emit review only. Earlier accepted Calendar light/dark compositions remain layout evidence, not jump-interaction evidence. No emulator data clear or native rebuild was used for recovery.
