...
- **RESOLVED — dark-mode invisible headline** (`eb6e0a0`). `TimelineScreen` headline + subhead had **no color** → defaulted to black → invisible on the dark canvas (light mode read fine). Bound to theme `ink` / `muted`. (A token-contrast audit can't catch a *missing* color — only looking does.)
...
- Exhaustive per-screen **dark-mode + cross-device visual pass** on an emulator (this pass fixed the one *invisible* case + verified visible fixes; a full sweep is still owner/QA work).
